...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = compare . map measure $ [a, b]
  where compare (x:y:[]) = if x > y then a else b
...
firstHalf x = case (odd . length ) x of
    True -> take (1 + (length x) `div` 2) x
    False -> take ((length x) `div` 2) x
...
joinToLength i strlist = filter (\x -> length x == i) [x ++ y | x <- strlist, y <- strlist]
